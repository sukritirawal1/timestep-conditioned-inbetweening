...
class FrameInterpolationEvaluator:
    ...
    def __init__(self, results_dir, device='cuda' if torch.cuda.is_available() else 'cpu'):
    ...
    def evaluate_sample(self, sample_dir):
        ...
        results['ssim'] = [
            self.ssim.compute(interps[i], targets[i]) for i in range(3)
        ]
    ...
    def compute_statistics(self, all_results):
        ...
        stats['lpips_overall_mean'] = np.mean([np.mean(r['lpips']) for r in all_results])
...
def main():
    # Configuration
    RESULTS_DIR = "data/optical_flow_results"
    NUM_SAMPLES = None  # Set to 5 for quick test, None for all
    ...
    evaluator.save_results(all_results, stats, 'optical_flow_evaluation.json')
