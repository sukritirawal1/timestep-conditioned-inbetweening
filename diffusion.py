...
class Diffusion:
    ...
    @torch.no_grad()
    def generate_inbetweens(self, start_frames, end_frames, num_inbetweens = 3, num_inference_steps = 25, noise_strength = 0.3):
        ...
        for i in range(1, num_inbetweens + 1):
            ...
            z_denoised = self.denoise_latent(z_interp, num_inference_steps, noise_strength)
...
def visualize_results(start_frames, end_frames, inbetweens, save_path=None):
    ...
    fig, axes = plt.subplots(num_sequences, total_frames, 
                            figsize=(3 * total_frames, 3 * num_sequences))
