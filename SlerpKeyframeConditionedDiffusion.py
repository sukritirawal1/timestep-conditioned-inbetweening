...
class SlerpKeyframeConditionedDiffusion(nn.Module):
    ...
    def training_step(self, batch, structure_loss = False):
        ...
        t = torch.randint(0, num_timesteps, (z_target.shape[0],), device=self.device).long()
    ...
    def val_metrics(self, val_loader, epoch, guidance_scale = 1.0, noise_strength = 0.3, num_inference_steps=25, structure_loss = False):
        self.eval()
        val_loss = 0.0
        with torch.no_grad():
    ...
    def train_model(self, train_loader, val_loader, num_epochs = 5, lr = 1e-4, save_dir = "model_ckpt", noise_strength = 0.3, num_denoising_steps = 25, structure_loss  = False, guidance_scale = 1.0):
        ...
        for epoch in range(num_epochs):
            ...
            val_loss = self.val_metrics(val_loader, epoch, guidance_scale=guidance_scale, noise_strength=noise_strength, num_inference_steps=num_denoising_steps, structure_loss=structure_loss)
    ...
    def predict_inbetween_sequence(self, start_frames, end_frames, num_inbetweens=3, num_inference_steps=25, noise_strength=0.3, guidance_scale=1.0):
        inbetween_frames = []
